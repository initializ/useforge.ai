...
Talk to a working demo agent in your terminal in under 60 seconds — no build, no cluster, no config. Scaffolds a keyless demo agent (the native forge LLM executor + the `weather` skill + `http_request`/`datetime_now`/`math_calculate` builtins) into a throwaway workspace, resolves whatever model credential is available, and drops you into a chat whose tool calls and egress checks render inline. See the [Quick Start](/docs/getting-started/quick-start) for the full walkthrough.
...
| `--with` | | Comma-separated channel adapters (e.g., `slack,telegram`). Gated by [settings](/docs/reference/settings) `channels.enabled` when set: a `--with` for a non-enabled adapter fails before the policy deny filter |
...
Add a channel adapter to the project. Refuses to scaffold an adapter not in [settings](/docs/reference/settings) `channels.enabled` when that allowlist is set.
...
Run a standalone channel adapter. Refuses to start an adapter not in [settings](/docs/reference/settings) `channels.enabled` when that allowlist is set (same gate as `forge run --with`), then applies the policy deny filter.
