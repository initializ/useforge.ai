...
```json
{
  ...
  "optimizer": { "enabled": true, "upstream": "https://gateway.corp/bedrock/claude" },
...
| `optimizer.enabled` | `bool` (tri-state) | Whether the [`forge` surface](/docs/reference/forge-surface) offers the context optimizer for the Claude Code path. **Off by default** (unset). Enabling it rewrites `ANTHROPIC_BASE_URL`, which can't be forced under enterprise-managed Claude Code, so it's opt-in. Resolved from **trusted layers only** (user / project-local / CLI / managed) — a checked-in project `.forge/settings.json` cannot enable it. Highest layer wins, so a managed layer can force-enable or force-disable. |
| `optimizer.upstream` | `string` (URL) | Base URL the optimizer proxy forwards to — typically the org's LLM gateway (e.g. a Kong/Bedrock endpoint). Consumed by `forge optimizer` / `forge optimizer start` (see [CLI reference](/docs/reference/cli-reference#forge-optimizer)). Resolution precedence: **managed settings > `--upstream` flag > `$FORGE_OPTIMIZER_UPSTREAM` > this (user) setting > `$ANTHROPIC_BASE_URL` chaining > Anthropic default**. Like `models.gateway`, it redirects Claude Code's traffic + Authorization header, so it is read from **trusted layers only** (the checked-in project layer is excluded); a **managed** value overrides even `--upstream` (enterprise enforcement). Validated as an http(s) URL at startup. |
...
- **`optimizer.enabled`**: consumed by the interactive [`forge` surface](/docs/reference/forge-surface). In the Claude Code path the optimizer option is **disabled/unselectable** unless a trusted layer set `optimizer.enabled: true` — a checked-in project layer is excluded (same base-url-redirect trust boundary as `models.gateway`). A **managed** layer is the enterprise-approved enablement path.
- **`optimizer.upstream`**: consumed by the `forge optimizer` proxy (foreground and the `start` daemon) as the base URL it forwards to. It slots into the upstream precedence between `$FORGE_OPTIMIZER_UPSTREAM` and `$ANTHROPIC_BASE_URL` chaining; a **managed** value outranks even `--upstream` so an org can pin the gateway. Read from trusted layers only (project layer excluded) because it redirects Claude Code's traffic and Authorization header. See [CLI reference → `forge optimizer`](/docs/reference/cli-reference#forge-optimizer).
